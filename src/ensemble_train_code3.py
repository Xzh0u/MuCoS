...
# TODO: deal with model copy in from pretrained!!!
...
class Ensemble3(BertPreTrainedModel):
    ...
    def forward(self, input_ids, attention_mask=None, token_type_ids=None, labels=None):
        ...
        x3 = self.sub_modules["modelC"](input_ids=input_ids, attention_mask=attention_mask,
                                        token_type_ids=token_type_ids)
        x = torch.cat((x1, x2[0], x3[0]), dim=2)
        # x = x.detach()
        logits = self.classifier(x)
        return logits
...
class RobertaClassificationHead3(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        self.dense = nn.Linear(
            config.hidden_size * 4, config.hidden_size * 4) # 2 + 1 + 1 
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
...
def main():
    ...
    model1 = Ensemble(modelX, modelX, config)
    model_copy = copy.deepcopy(model1) # in order to get the model structure

    model1 = model_class.from_pretrained(args.pred_modelA_dir, model_copy=model_copy, ensemble=True)
    model2 = model_class.from_pretrained(args.pred_modelB_dir)
    model3 = model_class.from_pretrained(args.pred_modelC_dir)
    ...
    model3.classifier = nn.Sequential(*list(model2.classifier.children())[:-1])

    model = Ensemble3(model1, model2, model3, config)
