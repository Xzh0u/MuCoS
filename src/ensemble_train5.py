...
class Ensemble(BertPreTrainedModel):
    def __init__(self, modelA, modelB, modelC, modelD, modelE, config):
    ...
    def forward(self, input_ids, attention_mask=None, token_type_ids=None, labels=None):
        ...
        x4 = self.modelD(input_ids=input_ids, attention_mask=attention_mask,
                                        token_type_ids=token_type_ids)
        x5 = self.modelE(input_ids=input_ids, attention_mask=attention_mask,
                                        token_type_ids=token_type_ids)
        x = torch.cat((x1[0], x2[0], x3[0], x4[0], x5[0]), dim=2)
...
class RobertaClassificationHead(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.dense = nn.Linear(
            config.hidden_size * 5, config.hidden_size * 5)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.out_proj = nn.Linear(config.hidden_size * 5, config.num_labels)
...
def main():
    ...
    parser.add_argument("--pred_modelD_dir", default=None, type=str,
                        help='./codesearch/models/java/checkpoint-best')
    parser.add_argument("--pred_modelE_dir", default=None, type=str,
                        help='./codesearch/models/java/checkpoint-best')
    ...
    modelE = model_class.from_pretrained(args.pred_modelE_dir)
    ...
    modelE.classifier = nn.Sequential(*list(modelE.classifier.children())[:-3])
    
    model = Ensemble(modelA, modelB, modelC, modelD, modelE, config)
